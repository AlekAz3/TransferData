...
namespace TransferData.Model.Services.Transfer
{
    public class TransferMSSQL : ITransfer
    {
        ...
        public string GenerateMergeQuery(string tableName)
        {
            ...
            var columns = schema.Fields.Select(x => x.FieldNameWithEscape(DbType.MSSQL)).ToList();
            ...
            sqlQueryString.AppendLine($"update set {schema.SetValuesSubQuery(DbType.MSSQL)} ");
            sqlQueryString.AppendLine($"when not matched then ");
            sqlQueryString.AppendLine($"insert ({string.Join(", ", columns)}) ");
            sqlQueryString.AppendLine($"values ({schema.ColumnsWithTableName(DbType.MSSQL)}) ");
        ...
        {
            var schema = _metadataExtractor.GetTableSchema(tableName);
            string columnsJoin = string.Join(", ", schema.Fields.Select(x => x.FieldNameWithEscape(Models.DbType.MSSQL)));
